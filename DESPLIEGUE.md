...
```
Vercel (estático)  ──HTTPS──▶  api.TU-DOMINIO  ──▶  Caddy ──▶ api ──┐
   tudominio.com                (Oracle Cloud)         worker  ─────┤──▶ Supabase
...
Un registro **A** de `api.TU-DOMINIO` a la IP pública de la instancia. Caddy pide el
...
```bash
CORS_ORIGINS=https://TU-DOMINIO            # el de Vercel; sin esto el navegador bloquea todo
PUBLIC_BASE_URL=https://api.TU-DOMINIO     # base de los webhooks de proveedor
...
```bash
export XFRAME_DOMAIN=api.TU-DOMINIO
...
```bash
curl https://api.TU-DOMINIO/health                    # {"status":"ok"}
curl -X POST https://api.TU-DOMINIO/chat              # 401
```
...
VITE_AGENT_URL=https://api.TU-DOMINIO
```
...
En **Authentication → URL Configuration**, añade `https://TU-DOMINIO` como Site URL y
...
proveedores llamarán a `https://api.TU-DOMINIO/webhooks/{proveedor}` y hay que darles de
